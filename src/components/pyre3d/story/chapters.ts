...
export function makeSandbox(seed: number): ChapterDef {
  return {
    ...
    index: 0,
    title: "Serbest Yıkım",
    subtitle: "Sonsuz yol, sınırsız yıkım",
    briefing: {
      lore:
        "Öne doğru sonsuz bir yol. Yol boyunca binalar, karşıdan gelen zeplinler " +
        "ve yerden elektrik çakan kuleler. Ne kadar ileri gidebilirsin?",
      objectives: ["Olabildiğince ileri git", "Hayatta kal"],
      tips: ["Tüm yetenekler açık", "W/S ile hız ayarı", "Q/E ile irtifa"],
    },
    world: {
      mode: "infinite",
      radius: 780,
      wind: { dir: Math.PI / 2, strength: 0.3 },
      start: { x: 0, y: 90, z: 0 },
      fogScale: 1.15,
    },
    objectives: [{ id: "survive", type: "survive", seconds: 99999, label: "Hayatta kal" }],
    fail: [{ type: "death" }],
    abilities: { flame: true, fireball: true, roll: true, shock: true, rage: true },
    par: { time: 99999, score: 999999 },
    rewardEmbers: 600,
    bondXp: 40,
    unlocks: [],
    beats: [],
